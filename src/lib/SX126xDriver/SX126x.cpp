...
#ifdef USE_SX1266_RFO_HF
  #ifndef OPT_USE_SX1266_RFO_HF
    #define OPT_USE_SX1266_RFO_HF true
  #endif
#else
  #define OPT_USE_SX1266_RFO_HF false
#endif
...
SX126xDriver::SX126xDriver(): SX12xxDriverCommon()
{
  ...
  currSyncWord = SX126X_SYNC_WORD;
  currBW =SX126x_BW_125_00_KHZ;
  currSF = SX126x_SF_7;
  currCR = SX126x_CR_4_5;
  currOpmode = SX126x_OPMODE_SLEEP;
  ModFSKorLoRa = SX126x_OPMODE_LORA;
  ...
  crcEnabled = false;
  lowFrequencyMode = SX1268_HIGH_FREQ;
...
bool SX126xDriver::Begin(uint32_t minimumFrequency, uint32_t maximumFrequency)
{
  ...
  {
    lowFrequencyMode = SX1268_LOW_FREQ;
    DBGLN("Setting 'lowFrequencyMode' used for 433MHz.");
  }

  SetMode(SX126x_OPMODE_STANDBY, SX12XX_Radio_All);
...
#endif
  {
    if (OPT_USE_SX1266_RFO_HF)
    {
      SetOutputPower(SX126X_MAX_OUTPUT_POWER_RFO_HF);
    }
    else
    {
      SetOutputPower(SX126X_MAX_OUTPUT_POWER);
    }
  }
  CommitOutputPower();
...
{
  SetMode(SX126x_OPMODE_SLEEP, SX12XX_Radio_All);
  hal.end();
  RFAMP.TXRXdisable();
  RemoveCallbacks();
}

void SX126xDriver::cwRepeat(SX12XX_Radio_Number_t radioNumber)
{
  uint8_t dummy[0] = {};

  ClearIrqFlags(radioNumber);

  hal.writeRegister(SX126X_REG_FIFO_ADDR_PTR, SX126X_FIFO_TX_BASE_ADDR_MAX, radioNumber);
  hal.writeRegister(SX126X_REG_FIFO, dummy, 0, radioNumber);

  SetMode(SX126x_OPMODE_TX, radioNumber);
}

void SX126xDriver::startCWTest(uint32_t freq, SX12XX_Radio_Number_t radioNumber)
{
  SetPreambleLength(8);
  SetSpreadingFactor(SX126x_SF_9);
  SetBandwidthCodingRate(SX126x_BW_125_00_KHZ, SX126x_CR_4_5);
  ...
  hal.writeRegister(SX126X_REG_PA_CONFIG, pwrCurrent, radioNumber);
...
{
  hal.writeRegister(SX126X_REG_OP_MODE, SX126x_OPMODE_SLEEP, SX12XX_Radio_All);
  hal.writeRegister(SX126X_REG_OP_MODE, ModFSKorLoRa, SX12XX_Radio_All); //must be written in sleep mode
  SetMode(SX126x_OPMODE_STANDBY, SX12XX_Radio_All);

  hal.writeRegister(SX126X_REG_PAYLOAD_LENGTH, PayloadLength, SX12XX_Radio_All);
  SetSyncWord(currSyncWord);
  hal.writeRegister(SX126X_REG_FIFO_TX_BASE_ADDR, SX126X_FIFO_TX_BASE_ADDR_MAX, SX12XX_Radio_All);
  hal.writeRegister(SX126X_REG_FIFO_RX_BASE_ADDR, SX126X_FIFO_RX_BASE_ADDR_MAX, SX12XX_Radio_All);
  hal.writeRegisterBits(SX126X_REG_DIO_MAPPING_1, SX126X_DIO0_RXTX_DONE, SX126X_DIO0_MASK, SX12XX_Radio_All); //undocumented "hack", looking at Table 18 from datasheet SX126X_REG_DIO_MAPPING_1 = 11 appears to be unspported by infact it generates an intterupt on both RXdone and TXdone, this saves switching modes.
  hal.writeRegister(SX126X_REG_LNA, SX126X_LNA_BOOST_ON, SX12XX_Radio_All);
  hal.writeRegister(SX1268_REG_MODEM_CONFIG_3, SX1268_AGC_AUTO_ON | SX1268_LOW_DATA_RATE_OPT_OFF, SX12XX_Radio_All);
  hal.writeRegisterBits(SX126X_REG_OCP, SX126X_OCP_ON | SX126X_OCP_150MA, SX126X_OCP_MASK, SX12XX_Radio_All); //150ma max current
  SetPreambleLength(SX126X_PREAMBLE_LENGTH_LSB);
}
...
void SX126xDriver::SetBandwidthCodingRate(SX126x_Bandwidth bw, SX126x_CodingRate cr)
{
  if ((currBW != bw) || (currCR != cr))
  {
    ...
    {
      hal.writeRegister(SX126X_REG_MODEM_CONFIG_1, bw | cr | SX126x_HEADER_IMPL_MODE, SX12XX_Radio_All);
      SetCRCMode(false);
    }
    else
    {
      if (headerExplMode)
      {
        hal.writeRegister(SX126X_REG_MODEM_CONFIG_1, bw | cr | SX126x_HEADER_EXPL_MODE, SX12XX_Radio_All);
      }
      else
      {
        hal.writeRegister(SX126X_REG_MODEM_CONFIG_1, bw | cr | SX126x_HEADER_IMPL_MODE, SX12XX_Radio_All);
    ...
    #if !defined(RADIO_SX1262) //does not apply to SX1262
      if (bw == SX126x_BW_500_00_KHZ)
      {
        ...
        hal.writeRegisterBits(SX126X_REG_DETECT_OPTIMIZE, 0x80, 0x80, SX12XX_Radio_All);
      }
      else
      {
        // Errata 2.1 Sensitivity Optimization with a 500 kHz Bandwidth https://www.semtech.com/products/wireless-rf/lora-connect/sx1276
        ...
        hal.writeRegisterBits(SX126X_REG_DETECT_OPTIMIZE, 0x00, 0x80, SX12XX_Radio_All);
      }
    #endif
...
void SX126xDriver::SetCRCMode(bool on)
{
  if(on)
  {
    #if defined(RADIO_SX1262)
      hal.writeRegisterBits(SX126X_REG_MODEM_CONFIG_1, SX1262_RX_CRC_MODE_ON, SX1262_RX_CRC_MODE_MASK, SX12XX_Radio_All);
    #else
      hal.writeRegisterBits(SX126X_REG_MODEM_CONFIG_2, SX1268_RX_CRC_MODE_ON, SX1268_RX_CRC_MODE_MASK, SX12XX_Radio_All);
    #endif
  }
  else
  {
    #if defined(RADIO_SX1262)
      hal.writeRegisterBits(SX126X_REG_MODEM_CONFIG_1, SX1262_RX_CRC_MODE_OFF, SX1262_RX_CRC_MODE_MASK, SX12XX_Radio_All);
    #else
      hal.writeRegisterBits(SX126X_REG_MODEM_CONFIG_2, SX1268_RX_CRC_MODE_OFF, SX1268_RX_CRC_MODE_MASK, SX12XX_Radio_All);
    #endif
  }
}
...
void SX126xDriver::SetSyncWord(uint8_t syncWord)
{
  ...
  hal.writeRegister(SX126X_REG_SYNC_WORD, _syncWord, SX12XX_Radio_All);
  currSyncWord = _syncWord;
}

/***
 * @brief: Schedule an output power change after the next transmit
 * The radio must be in SX126x_OPMODE_STANDBY to change the power
 ***/
void SX126xDriver::SetOutputPower(uint8_t Power)
{
  uint8_t pwrNew;
  Power &= SX126X_PA_POWER_MASK;

  if (OPT_USE_SX1266_RFO_HF)
  {
    pwrNew = SX126X_PA_SELECT_RFO | Power;
  }
  else
  {
    pwrNew = SX126X_PA_SELECT_BOOST | Power;
  }
...
void ICACHE_RAM_ATTR SX126xDriver::CommitOutputPower()
{
  ...
  hal.writeRegister(SX126X_REG_PA_CONFIG, pwrCurrent, SX12XX_Radio_All);
}
...
void SX126xDriver::SetPreambleLength(uint8_t PreambleLen)
{
  ...
  {
    hal.writeRegister(SX126X_REG_PREAMBLE_LSB, PreambleLen, SX12XX_Radio_All);
...
void SX126xDriver::SetSpreadingFactor(SX126x_SpreadingFactor sf)
{
  if (currSF != sf)
  {
    hal.writeRegisterBits(SX126X_REG_MODEM_CONFIG_2, sf | SX126X_TX_MODE_SINGLE,  SX126X_SPREADING_FACTOR_MASK | SX126X_TX_MODE_MASK, SX12XX_Radio_All);
    if (sf == SX126x_SF_6)
    {
      hal.writeRegisterBits(SX126X_REG_DETECT_OPTIMIZE, SX126X_DETECT_OPTIMIZE_SF_6, SX126X_DETECT_OPTIMIZE_SF_MASK, SX12XX_Radio_All);
      hal.writeRegister(SX126X_REG_DETECTION_THRESHOLD, SX126X_DETECTION_THRESHOLD_SF_6, SX12XX_Radio_All);
    }
    else
    {
      hal.writeRegisterBits(SX126X_REG_DETECT_OPTIMIZE, SX126X_DETECT_OPTIMIZE_SF_7_12, SX126X_DETECT_OPTIMIZE_SF_MASK, SX12XX_Radio_All);
      hal.writeRegister(SX126X_REG_DETECTION_THRESHOLD, SX126X_DETECTION_THRESHOLD_SF_7_12, SX12XX_Radio_All);
    }
    currSF = sf;
  }
}
...
{
  currFreq = regfreq;
  SetMode(SX126x_OPMODE_STANDBY, radioNumber);
  ...
  hal.writeRegister(SX126X_REG_FRF_MSB, outbuff, sizeof(outbuff), radioNumber);
}

void ICACHE_RAM_ATTR SX126xDriver::SetRxTimeoutUs(uint32_t interval)
{
  ...
  if (interval)
  {
    ...
    hal.writeRegisterBits(SX126X_REG_SYMB_TIMEOUT_MSB, timeoutSymbols >> 8, SX126X_REG_SYMB_TIMEOUT_MSB_MASK, SX12XX_Radio_All);  // set the timeout MSB
    hal.writeRegister(SX126X_REG_SYMB_TIMEOUT_LSB, timeoutSymbols & 0xFF, SX12XX_Radio_All);
...
bool SX126xDriver::DetectChip(SX12XX_Radio_Number_t radioNumber)
{
  ...
  while ((i < 3) && !flagFound)
  {
    uint8_t version = hal.readRegister(SX126X_REG_VERSION, radioNumber);
    DBG("%x", version);
    if (version == SX126X_VERSION)
    {
      flagFound = true;
    }
    else
  ...
  {
    DBGLN(" found! (match by REG_VERSION == 0x%x", SX126X_VERSION);
  }
  return true;
}
...
void ICACHE_RAM_ATTR SX126xDriver::TXnbISR()
{
  ...
  //TXdoneMicros = micros();
  // The power level must be changed when in SX126x_OPMODE_STANDBY, so this lags power
...
void ICACHE_RAM_ATTR SX126xDriver::TXnb(uint8_t * data, uint8_t size, SX12XX_Radio_Number_t radioNumber)
{
  ...
  RFAMP.TXenable(radioNumber);
  hal.writeRegister(SX126X_REG_FIFO_ADDR_PTR, SX126X_FIFO_TX_BASE_ADDR_MAX, radioNumber);
  hal.writeRegister(SX126X_REG_FIFO, data, size, radioNumber);
...
{
  uint8_t const FIFOaddr = hal.readRegister(SX126X_REG_FIFO_RX_CURRENT_ADDR, radioNumber);
  hal.writeRegister(SX126X_REG_FIFO_ADDR_PTR, FIFOaddr, radioNumber);
  hal.readRegister(SX126X_REG_FIFO, RXdataBuffer, PayloadLength, radioNumber);
...
void ICACHE_RAM_ATTR SX126xDriver::RXnb()
{
  ...
  if (timeoutSymbols)
  {
    SetMode(SX126x_OPMODE_RXSINGLE, SX12XX_Radio_All);
  }
  else
  {
    SetMode(SX126x_OPMODE_RXCONTINUOUS, SX12XX_Radio_All);
  }
}
...
void ICACHE_RAM_ATTR SX126xDriver::GetLastPacketStats()
{
  ...
  if (instance->isFirstRxIrq && GPIO_PIN_NSS_2 != UNDEF_PIN)
  {
    ...
    if(secondIrqStatus & SX126X_CLEAR_IRQ_FLAG_RX_DONE)
    {
      WORD_ALIGNED_ATTR uint8_t RXdataBuffer_second[RXBuffSize];
      uint8_t const FIFOaddr = hal.readRegister(SX126X_REG_FIFO_RX_CURRENT_ADDR, radio[secondRadioIdx]);
      hal.writeRegister(SX126X_REG_FIFO_ADDR_PTR, FIFOaddr, radio[secondRadioIdx]);
      hal.readRegister(SX126X_REG_FIFO, RXdataBuffer_second, PayloadLength, radio[secondRadioIdx]);
...
void ICACHE_RAM_ATTR SX126xDriver::SetMode(SX126x_RadioOPmodes mode, SX12XX_Radio_Number_t radioNumber)
{
  /*
  ...
  */
  ...
  hal.writeRegister(SX126X_REG_OP_MODE, mode | lowFrequencyMode, radioNumber);
  currOpmode = mode;
}

void SX126xDriver::Config(uint8_t bw, uint8_t sf, uint8_t cr, uint32_t freq, uint8_t preambleLen, bool InvertIQ, uint8_t _PayloadLength, uint32_t interval)
{
  Config(bw, sf, cr, freq, preambleLen, currSyncWord, InvertIQ, _PayloadLength, interval);
}

void SX126xDriver::Config(uint8_t bw, uint8_t sf, uint8_t cr, uint32_t freq, uint8_t preambleLen, uint8_t syncWord, bool InvertIQ, uint8_t _PayloadLength, uint32_t interval)
{
  PayloadLength = _PayloadLength;
  ConfigLoraDefaults();
  SetPreambleLength(preambleLen);
  SetSpreadingFactor((SX126x_SpreadingFactor)sf);
  SetBandwidthCodingRate((SX126x_Bandwidth)bw, (SX126x_CodingRate)cr);
...
void ICACHE_RAM_ATTR SX126xDriver::SetPPMoffsetReg(int32_t offset)
{
  ...
  hal.writeRegister(SX126x_PPMOFFSET, (uint8_t)offsetPPM, processingPacketRadio);
}

bool ICACHE_RAM_ATTR SX126xDriver::GetFrequencyErrorbool()
{
  return (hal.readRegister(SX126X_REG_FEI_MSB, processingPacketRadio) & 0b1000) >> 3; // returns true if pos freq error, neg if false
}
...
int32_t ICACHE_RAM_ATTR SX126xDriver::GetFrequencyError()
{
  ...
  hal.readRegister(SX126X_REG_FEI_MSB, reg, sizeof(reg), processingPacketRadio);
...
uint8_t ICACHE_RAM_ATTR SX126xDriver::UnsignedGetLastPacketRSSI(SX12XX_Radio_Number_t radioNumber)
{
  return hal.readRegister(SX126X_REG_PKT_RSSI_VALUE, radioNumber);
}

int8_t ICACHE_RAM_ATTR SX126xDriver::GetLastPacketRSSI(SX12XX_Radio_Number_t radioNumber)
{
  return ((lowFrequencyMode ? -164 : -157) + hal.readRegister(SX126X_REG_PKT_RSSI_VALUE, radioNumber));
}

int8_t ICACHE_RAM_ATTR SX126xDriver::GetCurrRSSI(SX12XX_Radio_Number_t radioNumber)
{
  return ((lowFrequencyMode ? -164 : -157) + hal.readRegister(SX126X_REG_RSSI_VALUE, radioNumber));
}

int8_t ICACHE_RAM_ATTR SX126xDriver::GetLastPacketSNRRaw(SX12XX_Radio_Number_t radioNumber)
{
  return (int8_t)hal.readRegister(SX126X_REG_PKT_SNR_VALUE, radioNumber);
}

uint8_t ICACHE_RAM_ATTR SX126xDriver::GetIrqFlags(SX12XX_Radio_Number_t radioNumber)
{
  return hal.readRegister(SX126X_REG_IRQ_FLAGS, radioNumber);
}

void ICACHE_RAM_ATTR SX126xDriver::ClearIrqFlags(SX12XX_Radio_Number_t radioNumber)
{
  hal.writeRegister(SX126X_REG_IRQ_FLAGS, SX126X_CLEAR_IRQ_FLAG_ALL, radioNumber);
}
...
void ICACHE_RAM_ATTR SX126xDriver::IsrCallback(SX12XX_Radio_Number_t radioNumber)
{
    ...
    if (irqStatus & SX126X_CLEAR_IRQ_FLAG_TX_DONE)
    {
        RFAMP.TXRXdisable();
        instance->TXnbISR();
        irqClearRadio = SX12XX_Radio_All;
    }
    else if (irqStatus & SX126X_CLEAR_IRQ_FLAG_RX_DONE)
    {
    ...
    else if (irqStatus == SX126X_CLEAR_IRQ_FLAG_NONE)
    {
        return;
    }
...
// {
//   SetMode(SX126X_STANDBY);

//   hal.writeRegisterBits(SX126X_REG_DIO_MAPPING_1, SX126X_DIO0_CAD_DONE | SX126X_DIO1_CAD_DETECTED, 7, 4);
